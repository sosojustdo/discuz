...
class logging_ctl {
    ...
    function on_login() {
        ...
        //伪代码 if(request里有当当登录flag)， 参数在request里取，然后调用userlogin
        //$result = userlogin($_GET['username'], $_GET['password'], 0, '', 'auto', $_G['clientip']);
        // if (request里没有当当登录flag)，那么继续原逻辑，在url里get参数，不影响管理员登录
        ...
        if($result['status'] > 0) {
            ...
            if(!$freeze || !$this->setting['accountguard']['loginpwcheck']) {
                ...
                if(!empty($_G['referer'])){
                    ...
                    $location = $invite || $_G['groupid'] == 8 ? 'home.php?mod=space&do=home' : dreferer().'forum.php';
                }
